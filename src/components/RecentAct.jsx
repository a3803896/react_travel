  ...
  return (
    <section className={`${className}`}>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 lg:gap-x-7.5 gap-y-4 lg:gap-y-3'>
        {dataArray.map((item) => {
          return (
            <div key={item.ID} className='activity_card flex lg:border lg:border-second-229 lg:rounded-xl lg:overflow-hidden'>
              <Link
                to={`/activityinfo/${item.ID}`}
                ...
                <img className='activity_card_logo w-full h-full object-cover' src={item.Picture.PictureUrl1} alt={item.Name} />
              </Link>
              <div className='flex-grow flex flex-col lg:bg-second-249 pl-4 pr-1 lg:px-7.5 lg:py-4'>
                <div className='lg:flex-grow'>
                  ...
                  <Link to={`/activityinfo/${item.ID}`} className='block mb-1/4 lg:mb-0'>
                    <h5 className='text-second-47 font-bold lg:text-1.5xl lg:leading-8 ellipsis-2'>{item.Name}</h5>
                  </Link>
                </div>
